...
int QuickSelect(vector<int> &arr, int left, int right, int k, size_t &comparisons, size_t &swaps, size_t group_size)
{
    if (left == right)
        return arr[left];
    ...
    {
        int subRight = min((int)(i + group_size - 1), right);
    ...
    if (medians.size() == 1)
        ...
        medianOfMedians = QuickSelect(medians, 0, medians.size() - 1, medians.size() / 2, comparisons, swaps, group_size);
    ...
    if (k == pivotIndex)
        ...
        return QuickSelect(arr, left, pivotIndex - 1, k, comparisons, swaps, group_size);
    else
        return QuickSelect(arr, pivotIndex + 1, right, k, comparisons, swaps, group_size);
}
