...
int test_insert_delete(vector<int> ikeys, vector<int> dkeys, size_t N, string testname, string ds)
{
    ...
    for (size_t i = 1; i < N; ++i)
    {
        ...
        root = splay_insert(root, ikeys[i], local_comparisons, local_pointer_operations);

        tree_heights.push_back(splay_height(root));
    ...
    for (size_t i = 0; i < N; ++i)
    {
        ...
        root = splay_remove(root, dkeys[i], local_comparisons, local_pointer_operations);
...
int main(const int argc, const char *argv[])
{
    ...
    test_insert_delete(ikeys_rand, dkeys_rand, N, "random_keys_insertion_and_deletion", "Splay");
    test_insert_delete(ikeys_asc, dkeys_rand, N, "ascending_keys_insertion_and_deletion", "Splay");
}
