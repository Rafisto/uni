...
using monomian_t = std::vector<double>;
using poly_t = std::unordered_map<monomian_t, double, container_hash<monomian_t>>;
using poly_order_t = bool (*)(const monomian_t&, const monomian_t&); 

inline char get_var_name(size_t index) {
    // static const char vars[] = {'u','v','x','y','z'}; // testing
    static const char vars[] = {'x','y','z'}; 
    return index < sizeof(vars) ? vars[index] : '?';
}

inline auto clean_num = [](double val) {
    return (std::abs(val - std::round(val)) < 1e-9) ? std::round(val) : val;
};

inline std::string poly_fmt(const poly_t& poly, poly_order_t compare) {
    if (poly.empty()) return "0";

    std::vector<std::pair<monomian_t, double>> terms(poly.begin(), poly.end());
