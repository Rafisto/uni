...
procedure Main is
   ...
   type MutexList is array (1 .. N_Philosophers) of Boolean;

   protected Table_Arbiter is
      procedure Atomic_Pick (Left, Right : in Integer; Success : out Boolean);
      procedure Atomic_Put (Left, Right : in Integer);
   private
      -- False - not taken
      -- True - taken
      Forks : MutexList := (others => False);
   end Table_Arbiter;

   protected body Table_Arbiter is
      procedure Atomic_Pick (Left, Right : in Integer; Success : out Boolean)
      is
      begin
         if not Forks (Left) and not Forks (Right) then
            Forks (Left) := True;
            Forks (Right) := True;
            Success := True;
         else
      ...
      procedure Atomic_Put (Left, Right : in Integer) is
      begin
         Forks (Left) := False;
         Forks (Right) := False;
      end Atomic_Put;
   ...
   begin
      ...
      for i in 1 .. N_Iterations loop
         ...
         Time := Random (G) * Time_Constraint;
         Put_Line("Philosopher(id" & Integer'Image (ID) & ", meal" & Integer'Image(i) & ") thinks for" & Float'Image (Time) & " sec.");

         delay Duration (Time);

         loop
            Table_Arbiter.Atomic_Pick (ForkLeft, ForkRight, Success);
         ...
         -- eating
         Time := Random (G) * Time_Constraint;
         Put_Line("Philosopher(id" & Integer'Image (ID) & ", meal" & Integer'Image(i) & ") eats for" & Float'Image (Time) & " sec.");

         delay Duration (Time);

         -- leave both forks
         Table_Arbiter.Atomic_Put (ForkLeft, ForkRight);
      end loop;
