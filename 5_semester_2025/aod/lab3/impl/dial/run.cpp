...
void run_ss(const std::string &gr_file, const std::string &ss_file,
            const std::string &oss_file) {
    ...
    while (!gr_file_stream.eof()) {
        ...
        if (line.starts_with("c")) {
            ...
            sscanf(line.c_str(), "a %lld %lld %lld", &from, &to, &weight);
    ...
    oss_file_stream << "g " << nodes << " " << edges << " " << min_cost << " "
                    << max_cost << "\n";
    oss_file_stream << "t " << avg_time << "\n";
}

void run_p2p(const std::string &gr_file, const std::string &p2p_file,
             const std::string &op2p_file) {
    std::ifstream gr_file_stream(gr_file);
    if (!gr_file_stream.is_open()) {
        std::cerr << "Error: Unable to open graph file: " << gr_file << "\n";
        return;
    ...
    std::vector<std::tuple<long long, long long>> pairs;
    ...
    if (pairs.size() == 0) pairs.emplace_back(1, nodes);
    ...
    for (const auto &[src, target] : pairs) {
        ...
        auto t0 = std::chrono::high_resolution_clock::now();
        dial.dial(graph, src, max_cost, target);
        auto t1 = std::chrono::high_resolution_clock::now();

        std::chrono::duration<double, std::milli> dt = t1 - t0;
        times.push_back(dt.count());

        long long distance = dial.getDistance(target);
        if (distance == INT_MAX)
            distance = -1;

        op2p_file_stream << "q " << src << " " << target << " d " << distance
                         << " t " << dt.count() << "\n";
    }
    ...
    op2p_file_stream << "g " << nodes << " " << edges << " " << min_cost << " "
                     << max_cost << " " << min_node << " " << max_node << "\n";
    op2p_file_stream << "t " << avg_time << "\n";
}

int main(int argc, char *argv[]) {
    if (argc < 6) {
        std::cerr << "Usage: dial -d <data_file> -ss <source_file> -oss "
                     "<output_file>\n";
        std::cerr << "Usage: dial -d <data_file> -p2p <pairs_file> -op2p "
                     "<output_file>\n";
        return 1;
    }
